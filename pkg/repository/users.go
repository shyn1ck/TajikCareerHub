...
func GetAllUsers() (users []models.User, err error) {
	err = db.GetDBConn().Preload("Role").Omit("password").Where("deleted_at = false").Find(&users).Error
	if err != nil {
...
func GetUserByID(id uint) (user models.User, err error) {
	err = db.GetDBConn().Preload("Role").Omit("password").Where("id = ? AND deleted_at = false", id).First(&user).Error
	if err != nil {
...
func GetUserByUsername(username string) (*models.User, error) {
	...
	err := db.GetDBConn().Preload("Role").Omit("password").Where("user_name = ? AND deleted_at = false", username).First(&user).Error
